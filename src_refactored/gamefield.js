...
lowfat.Gamefield = function (scene, spriteFactory, gameStateModel, soundManager, screenSize) {
    ...
    var ROWS = 7;
    var MAX_VALUE = 10;
    var BLOCK_SPRITE_WIDTH = 56;
    var BOTTOM_MARGIN = 40;
    var LEFT_MARGIN = 0;
    var PACK_Y = 7;
    var PACK_LENGTH = 2;
    var DEFAULT_PACK_X = 2;
    var BLOCK_FALL_SPEED_DROP = 0.05;
    var BLOCK_FALL_SPEED_GRAVITY = 0.05;
    var BLOCK_MERGE_DURATION = 0.05;
    ...
    function initLayers() {
        bgGradient = new cc.LayerGradient(cc.color(15, 43, 63), cc.color(17, 111, 115));
        ...
        ingameUI = new lowfat.IngameUI(this, spriteFactory, soundManager, getScoreUI, processNewGame);
        ingameUI.init(uiContainer);
        floatingScore = lowfat.FloatingScore(uiContainer, fgContainer);
        sideMenu = new lowfat.SideMenu(this, spriteFactory, soundManager);
        sideMenu.init(uiContainer);
    }

    function initBoard() {
        board = lowfat.GameBoard(this, COLS, ROWS, MAX_VALUE, onNoMatchesFoundWithDelay, processChainMergesWithDelay, getMaxUnlockedValue, setMaxUnlockedValue, showFallingBlocks);
    ...
    function initControls() {
        controlSchemes = [];
        if (cc.sys.isMobile) {
            controlSchemes.push(lowfat.TouchControls(this, movePackLeft, movePackRight, dropPack, swapPack, getInputIsLocked));
        } else {
            controlSchemes.push(lowfat.KeyboardControls(this, movePackLeft, movePackRight, dropPack, swapPack, getInputIsLocked, getScene, getBlockPreviews));
            controlSchemes.push(lowfat.DesktopMouseControlsNoDrag(this, dropPack, swapPack, BLOCK_SPRITE_WIDTH, LEFT_MARGIN, COLS, ROWS,
    ...
    function createPreviews() {
        for (var i = 0; i < currentPack.length; i++) {
            if (blockPreviews.length <= i) {
                var blockPreview = spriteFactory.getSprite("BlockPreviewAlternative");
    ...
    function dropPack () {
        ...
        for (var i = 0; i < topBlocks.length; i++) {
            ...
            var toY = board.getColHeight(toX);
    ...
    function restart() {
        ...
        score = 0;
        scoreUI.reset();
        scoreUI.showInitial(score, gameStateModel.highScore);
    ...
    function onResize(screenSize) {
        ...
        // bgParticles.setPosition(screenSizeInPoints.width / 2, 360);
        // bgParticles.setPosVar({x: screenSizeInPoints.width / 2, y: 360});
        // bgParticles.setTotalParticles(Math.ceil(screenSizeInPoints.width / 25));
    }
...
lowfat.GameBoard = function (gamefield, boardWidth, boardHeight, maxValue, onNoMatchesFoundWithDelay, processChainMergesWithDelay, getMaxUnlockedValue, setMaxUnlockedValue, showFallingBlocks) {
    var COLS = boardWidth;
...
lowfat.ScoreUI = function (container) {
    ...
    var COORDS_NORMAL_SCORE = {x: 150, y: 690};
    var SCALE_NORMAL_SCORE = 1;
    var COORDS_NORMAL_HISCORE = {x: 150, y: 640};
    var SCALE_NORMAL_HISCORE = 0.5;
    var COORDS_CENTERED_SCORE = {x: 150, y: 480};
    var SCALE_CENTERED_SCORE = 1.3;
    ...
    function moveToNormalPosition () {
        ...
        var hiScoreMoveAction = new cc.MoveTo(duration, COORDS_NORMAL_HISCORE.x, COORDS_NORMAL_HISCORE.y).easing(cc.easeCubicActionOut());
...
lowfat.TouchControls = function (gamefield, movePackLeft, movePackRight, dropPack, swapPack, getInputIsLocked) {
    ...
    function processTouchStarted (touchX, touchY) {
        if (getInputIsLocked() || !gamefield.getSideMenu().processClickAndGetIfAllowed(touchX, touchY)) {
            return;
        }
...
lowfat.KeyboardControls = function (gamefield, movePackLeft, movePackRight, dropPack, swapPack, getInputIsLocked, getScene, getBlockPreviews) {
    function init () {
        addListeners();
    }
...
lowfat.DesktopMouseControlsNoDrag = function (gamefield, dropPack, swapPack, BLOCK_SPRITE_WIDTH, LEFT_MARGIN, COLS, ROWS,
                                              getScene, getInputIsLocked, getSideMenu, getFgContainer, pixelsToCellX, pixelsToCellY,
                                              setNextPackX, getBlockPreviews, getTopBlocks, cellToPixelsX, updateBlockPreviews) {
